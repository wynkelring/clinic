...
public class VisitService implements IVisitService {
  ...
  public VisitService(
      VisitHoursService visitHoursService,
    ...
    if (isBookingAvailable(visitHoursId, numberInQueue, user.getId())
      ...
      emailService.sendEmail(
          ...
          messageSource.getMessage("email.content.book", null, LocaleContextHolder.getLocale())
              + "\n"
              + visit.getVisitHours().getDoctor().getFirstName()
              + " "
              + visit.getVisitHours().getDoctor().getFirstName()
              + "\n"
              + visit
                  .getVisitHours()
                  .getStartDate()
                  .plusMinutes(visit.getVisitHours().getVisitLength() * visit.getNumberInQueue())
              + "\n"
  ...
  public boolean isBookingAvailable(Long visitHoursId, Integer numberInQueue, Long userId) {
    ...
    if (visitHours != null) {
      LocalDateTime visitDate =
          visitHours.getStartDate().plusMinutes(visitHours.getVisitLength() * (numberInQueue - 1));
      return (!visitRepository.existsByCancelledIsFalseAndVisitHoursIdAndNumberInQueue(
              visitHoursId, numberInQueue)
          && visitDate.isAfter(LocalDateTime.now())
          && !visitRepository.existsByCancelledIsFalseAndVisitHoursIdAndPatientId(
              visitHoursId, userId));
    }
    return false;
  }
  ...
  public Page<Visit> getVisitsPageForDoctor(Long doctorId, Pageable pageable) {
    return visitRepository.findAllByVisitHoursDoctorIdOrderByVisitHoursStartDate(
        doctorId, pageable);
  }
  ...
  public void cancelVisitForDoctor(Long visitId, Long doctorId) {
    Optional<Visit> optVisit =
        visitRepository.findByIdAndVisitHoursDoctorIdAndCancelledIsFalse(visitId, doctorId);
    optVisit.ifPresent(
        visit -> {
          visit.setCancelled(true);
          visitRepository.save(visit);
  ...
  public void cancelVisitForPatient(Long visitId, Long patientId) {
    Optional<Visit> optVisit =
        visitRepository.findByIdAndPatientIdAndCancelledIsFalse(visitId, patientId);
    optVisit.ifPresent(
        visit -> {
          visit.setCancelled(true);
          visitRepository.save(visit);
  ...
  public void approveVisitsOnEndOfMonth(LocalDateTime now) {
    List<Visit> visits =
        visitRepository.findAllByCancelledIsFalseAndApprovedIsFalseAndVisitHoursEndDateBefore(
            LocalDateTime.now());
    visits.forEach(
        visit -> {
          visit.setApproved(true);
  ...
  private void sendApprovedEmail(Visit visit) {
    emailService.sendEmail(
        ...
        messageSource.getMessage("email.content.approved", null, LocaleContextHolder.getLocale())
            ...
            + visit.getVisitHours().getDescription()
            + "\n\n"
            + "http://localhost:8080/visits/invoice/"
            + visit.getId());
  }
}
