...
public class VisitHoursController {
  ...
  public VisitHoursController(
      VisitHoursService visitHoursService, UserService userService, VisitService visitService) {
  ...
  public String getVisitHours(
      Model model, @RequestParam(value = "notFound", required = false) String notFound) {
    if (notFound != null) {
  ...
  @PostMapping()
  public String searchVisitHours(
      @Valid @ModelAttribute("searchVisitHours") SearchVisitHoursDTO visitHours, Model model) {
    model.addAttribute("doctorsList", userService.getDoctorsList());
    model.addAttribute("searchVisitHours", visitHours);
    model.addAttribute(
        "list", visitHoursService.getVisitHoursNotCancelledByDoctor(visitHours.getDoctorId()));
  ...
  public String getVisitHour(
    ...
    if (visitHours != null && visitHours.getEndDate().isAfter(LocalDateTime.now())) {
      ...
      for (int i = 0; i < visitHours.getVisitsCount(); i++) {
        ...
        hoursArray.add(
            visitHours.getStartDate().plusMinutes(i * visitHours.getVisitLength()).toString());
        hoursArray.add(
            visitHours
                .getStartDate()
                .plusMinutes((i + 1) * visitHours.getVisitLength())
                .toString());
  ...
  @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_DOCTOR')")
  @GetMapping("/add")
  public String addVisitHours(
      @RequestParam(value = "errorDate", required = false) String errorDate, Model model) {
    if (errorDate != null) {
  ...
  @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_DOCTOR')")
  @PostMapping("/add")
  public String addVisitHoursPost(
      @Valid @ModelAttribute("addVisitHours") VisitHoursDTO visitHours,
    ...
    if (result.getErrorCount() == 0) {
      ...
      if (visitHoursService.hasDoctorVisitingHours(
          ...
          visitHours
              .getStartDate()
              .plusMinutes(visitHours.getVisitLength() * visitHours.getVisitsCount()))) {
