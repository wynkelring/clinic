...
public class PdfService {
  public void generateInvoice(Visit visit, HttpServletResponse response) {
    try {
      ...
      pdf.open();
      Paragraph title =
          new Paragraph("Invoice", new Font(Font.FontFamily.TIMES_ROMAN, 30f, Font.BOLD));
      ...
      visitTable.addCell(
          visit
              .getVisitHours()
              .getStartDate()
              .plusMinutes((visit.getNumberInQueue() - 1) * visit.getVisitHours().getVisitLength())
              .toString());
      ...
      visitTable.addCell(
          visit.getVisitHours().getDoctor().getFirstName()
              + " "
              + visit.getVisitHours().getDoctor().getLastName());
